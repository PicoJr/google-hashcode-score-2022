...
use crate::data::{PContributorSkill, PProject, PContributor, PInput};
...
fn contributor_skill(input: &str) -> Res<&str, PContributorSkill> {
    let (i, (skill_name, _, skill_level)) = tuple((
...
fn contributor(input: &str) -> Res<&str, PContributor> {
    let (i, (name, _, n_skills)) = terminated(tuple((non_space_or_unix_eol, single_space, positive_number)), line_ending)(input)?;
...
fn _parse_input(input: &str) -> Res<&str, PInput> {
    let (i, (n_contributors, _, n_projects)) = terminated(tuple((positive_number, single_space, positive_number)), line_ending)(input)?;
    let (i, contributors) = many_m_n(
        n_contributors,
        n_contributors,
            contributor
    )(i)?;
    let (i, projects) = many_m_n(
        n_projects,
        n_projects,
            project
    )(i)?;
